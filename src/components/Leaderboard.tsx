...
export const Leaderboard = () => {
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default:
        return <Trophy className="w-6 h-6 text-blue-400 opacity-50" />;
    }
  };
  ...
  return (
    <div className="bg-gradient-to-br from-[#1E2A37]/80 to-[#1E2A37]/50 backdrop-blur-lg rounded-3xl p-6 md:p-8 text-white relative overflow-hidden border border-white/5 h-full shadow-xl">
      <div className="absolute inset-0 bg-gradient-to-br from-[#2D9CDB]/5 to-transparent" />
      
      <div className="relative">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-bold bg-gradient-to-r from-[#2D9CDB] to-[#7F56D9] bg-clip-text text-transparent mb-2">
            Top Contributors
          </h2>
          <p className="text-gray-400">Leading members of our community</p>
        </div>
        ...
        <div className="space-y-4">
          {isLoading ? (
            [...Array(5)].map((_, i) => (
              <div 
                key={i} 
                className="bg-[#1E2A37] rounded-xl p-4 animate-pulse"
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#2D9CDB]/10 rounded-xl" />
                  <div className="flex-1">
                    <div className="h-4 bg-[#2D9CDB]/10 rounded w-24 mb-2" />
                    <div className="h-3 bg-[#2D9CDB]/10 rounded w-32" />
                  </div>
                  <div className="text-right">
                    <div className="h-4 bg-[#2D9CDB]/10 rounded w-20 mb-2" />
                    <div className="h-3 bg-[#2D9CDB]/10 rounded w-16" />
                  </div>
                </div>
              </div>
            ))
          ) : topDepositors.length === 0 ? (
            <div className="bg-[#1E2A37] rounded-xl p-8 text-center">
            ...
            topDepositors.map((contributor, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`bg-[#1E2A37] rounded-xl p-4 flex items-center gap-4 border ${styles.border} transition-all duration-200 ${styles.hover} shadow-lg`}
                >
